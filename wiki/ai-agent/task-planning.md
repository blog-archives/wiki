...
> **Status: Disputed** — 上段“唯一的结构化例外”只适合原文讨论的子任务协调范围，不能扩展到全部任务管理。新增材料展示 Codex 还有持久化的 Goal 目标、预算与状态；Plan 本身也有结构化工具参数，但这不等于执行器按步骤自动推进。详见 [任务状态：让目标与进度跨越上下文窗口](context-engineering/task-state.md)，来源为 [任务状态源码问答](../raw/2026-09-22-153920.md)。
...
- [多 Agent](multi-agent/index.md) — 各项目 agent 角色的预定义、触发与委派。
