...
description: 用问题链推进的中文技术写作规范。写作或修改技术文档、知识库文章、源码机制解析、设计说明、评审稿、分享稿时使用；尤其当需要让读者顺着「提出问题 → 给出解决思路 → 暴露更深的问题 → 继续解决」这条线索读下去，或需要把语气约束得平稳、判断有根据、不写成宣传稿或散文时。
---
...
文章不该是「问题清单 + 模块清单」，而是一条链。**每一节先回答上一节留下的问题，回答的过程再暴露出一个更深的问题，把读者推到下一节。** 推进力来自问题本身，而不是章节编号或 `接下来我们看……` 这类过场。

**每一步都要交代「为什么现在讲它」。** 检验方法是看这一节去掉之后，后文还能不能读懂：如果仍然读得通，说明它没落在链条上，应当并入相邻环节，或压缩成就近的简短说明。

让链条成立的四条规则：

1. **方向性推进。** 每一节都要制造下一节要解决的问题。三件事实并列、顺序可以随意调换的写法，就是方向断了；改成后一节依赖前一节。
2. **先铺问题，再给方案。** 方案要等问题的深度和代价都讲清楚之后才出场。先问自己：我讲清楚它为什么难了吗？我交代了不解决的代价吗？
3. **写出完整弧线。** 一个方案通常沿这条线推进：`问题 -> 压力 -> 根因 -> 旧做法为什么失效 -> 新做法为什么成立 -> 怎么落地`。不必每篇都写全，但缺哪一段，读者就会在哪一段存疑。
4. **接缝自查。** 每次从一节转到下一节，回答三个问题：为什么转到这里？上一环闭合了吗？这是不是自然的下一步？转折生硬，通常是上一环没闭合。

这条链和上面的主线并不冲突：`提出猜想` 是抛出问题，`分节解答` 是逐环回答，`回指前文` 是确认链条没有断。
...
**只有当整篇内容确实能归拢成一句话时，才用 `==...==` 把这句话标出来。** 它服务的是「全篇的结论」，不是「这一段里最要紧的一句」；一段特别关键，加粗就够了。多数文章达不到这个门槛，就不要加。

约束：

- **先问「能不能一句话归拢全篇」；不能就不加，拿不准默认不加。**
- **一篇最多一处**，偶尔两处。
- **不叠用**：同一句不要既加粗又高亮。
- **高亮判断，不堆名词**：应是一句可判断真假的结论。

**反直觉的内容不要用高亮，改用别的方式突出**，否则高亮就不稀缺了：

- **先设想，再纠正**：先写出读者可能的默认理解，再给出原文要求，句法见 `Reader-Aware Explanations`；
- **对比句**：把常见做法和原文要求并排放，让差异自己显现；
- **单独成段**：把那一句从长段落里拎出来，前后留白；
- **加粗后就地说明差在哪**：不要只留一句结论，下一句交代它和直觉差在哪里。
...
- 把每节标题按顺序读一遍，能不能看出是一条问题链，而不是一组并列主题
- 有没有哪一节去掉之后，后文仍然读得通（读得通说明它没落在链条上）
- 方案出现之前，问题有没有被铺到足够重
- 指标名、动作词、状态词有没有混用，名词和动词是不是同一套语义系统
- 有没有过场句在替内容占位置；如果删掉一句话不影响论证，就直接删掉，不改写成另一句过场话
- 中文引号有没有统一成 `「」`
- 有没有把高亮当第二个加粗用：是不是「全篇一句话」，有没有超过两处，有没有与加粗叠在同一句；反直觉内容有没有改用对比或「先设想再纠正」
...
4. 检查每一节是否回答了上一节留下的问题，并在结尾逼出一个更深的问题

如果需要给出改写建议，优先直接改成可用的句子，不要只做抽象点评。

## Sources

本 skill 由两个上游 skill 合并而成：

- [luoling8192/technical-writing](https://github.com/luoling8192/technical-writing)（MIT，Copyright (c) 2026 RainbowBird）：提供中文内部技术文档的语气、句法、结构约束与 Buzzword Blocklist，是本文件的主体。
- [Xamfonos/technical-writing-best-practices](https://github.com/Xamfonos/technical-writing-best-practices)（MIT）：提供「问题链」的四条结构规则（`问题链：让每一节逼出下一节`），中文表述为本地改写。

修改上游内容时，请保留 LICENSE 与本节署名。
